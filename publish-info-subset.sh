...
if [ -f output/miner_info/json_latest_subset/_SUCCESS ] ; then
  ...
  cat $PART | jq -s "{ date: \"$DATE\", miners: map({ key: .miner, value: { sectorSize: .[\"last(sectorSize)\"], peerId: .[\"last(peerId)\"], multiaddrsDecoded: .[\"last(multiaddrsDecoded)\"] } }) | from_entries }" > dist/miner-info-subset-latest/miner-info-subset-latest.json
fi
