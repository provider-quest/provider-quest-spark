...
def process(deals, providerCountryStateProvinces, suffix=""):
    ...
    dealsWithRegions = deals \
        .join(
            ...
            deals.provider == providerCountryStateProvinces.provider,
            how='leftOuter'
        ) \
        .fillna('none', 'region') \
        .drop(providerCountryStateProvinces.provider)
    ...
    #    .writeStream \
    #    .queryName("deals_by_synthetic_csp_region_json") \
    #    .format("json") \
    #    .option("path", outputDir + "/deals/by_synthetic_csp_region/archive/json") \
    #    .option("checkpointLocation", checkpointDir + "/deals/by_synthetic_csp_region/archive/json") \
    ...
    queryAggrDealsByRegionByProviderDaily = dealsDailyAggrByRegionByProvider \
        .writeStream \
        .queryName("deals_aggr_by_synthetic_csp_region_by_provider_daily_with_splits_json") \
        .format("json") \
        .option("path", outputDir + "/deals/by_synthetic_csp_region/by_provider/aggr_daily_with_splits/json") \
        .option("checkpointLocation", checkpointDir + "/deals/by_synthetic_csp_region/by_provider/aggr_daily_with_splits/json") \
        .partitionBy("region", "date") \
        .trigger(processingTime='1 minute') \
        .start()
    ...
    def output_summed(df, epoch_id):
        ...
        summedDf.orderBy('date', 'region').coalesce(1).write.json(
            outputDir + '/deals/by_synthetic_csp_region/sum_aggr_daily/json',
            mode='overwrite')

    queryDealsSumAvgDaily = dealsDailyAggrByRegionByProvider \
        .writeStream \
        .queryName("deals_by_synthetic_csp_region_sum_aggr_daily_json") \
        .outputMode('complete') \
        .option("checkpointLocation", checkpointDir + "/deals/by_synthetic_csp_region/sum_aggr_daily/json") \
    ...
    queryAggrDealsByRegionDaily = dealsDailyAggrByRegion \
        .writeStream \
        .queryName("deals_aggr_by_synthetic_csp_region_daily_json") \
        .format("json") \
        .option("path", outputDir + "/deals/by_synthetic_csp_region/aggr_daily/json") \
        .option("checkpointLocation", checkpointDir + "/deals/by_synthetic_csp_region/aggr_daily/json") \
        .trigger(processingTime='1 minute') \
        .start()
