...
def process(minerPower, syntheticRegions, suffix=""):
    ...
    queryArchiveDealsByRegion = minerPowerWithRegions \
        .writeStream \
        .queryName("miner_power_by_synthetic_csp_region_json") \
        .format("json") \
        .option("path", outputDir + "/miner_power/by_synthetic_csp_region/archive/json") \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_synthetic_csp_region/archive/json") \
    ...
    queryPowerAvgDaily = averagePowerDaily \
        .writeStream \
        .queryName("miner_power_by_synthetic_csp_region_avg_daily_json") \
        .format("json") \
        .option("path", outputDir + "/miner_power/by_synthetic_csp_region/avg_daily/json") \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_synthetic_csp_region/avg_daily/json") \
        .partitionBy("date") \
        .trigger(processingTime='1 minute') \
        .start()
    ...
    def output_summed(df, epoch_id):
        ...
        summedDf.orderBy('date', 'region').coalesce(1).write.json(
            outputDir + '/miner_power/by_synthetic_csp_region/sum_avg_daily/json',
            mode='overwrite')

    queryPowerSumAvgDaily = averagePowerDaily \
        .writeStream \
        .queryName("miner_power_by_synthetic_csp_region_sum_avg_daily_json") \
        .outputMode('complete') \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_synthetic_csp_region/sum_avg_daily/json") \
