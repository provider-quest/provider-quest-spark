...
def process(minerPower, providerCountryStateProvinces, suffix=""):
    ...
    queryArchiveDealsByRegion = minerPowerWithRegions \
        ...
        .format("json") \
        .option("path", outputDir + "/miner_power/by_provider_country_state_province/archive/json") \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_provider_country_state_province/archive/json") \
    ...
    queryPowerAvgDaily = averagePowerDaily \
        ...
        .format("json") \
        .option("path", outputDir + "/miner_power/by_provider_country_state_province/avg_daily/json") \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_provider_country_state_province/avg_daily/json") \
        .partitionBy("date") \
        .trigger(processingTime='1 minute') \
        .start()
    ...
    def output_summed(df, epoch_id):
        ...
        summedDf.orderBy('date', 'region').coalesce(1).write.json(
            outputDir + '/miner_power/by_provider_country_state_province/sum_avg_daily/json',
            mode='overwrite')
    ...
    queryPowerSumAvgDaily = averagePowerDaily \
        ...
        .outputMode('complete') \
        .option("checkpointLocation", checkpointDir + "/miner_power/by_provider_country_state_province/sum_avg_daily/json") \
