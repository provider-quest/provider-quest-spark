...
mkdir -p $WORK_DIR/dist/miner-power-daily-average-latest
...
(
  ...
  done) | jq -s "{ date: \"$DATE\", miners: map({ key: .miner, value: { qualityAdjPower: .qualityAdjPower, rawBytePower: .rawBytePower } }) | from_entries }" > $WORK_DIR/dist/miner-power-daily-average-latest/miner-power-daily-average-latest.json
